...
ServoPos1 = (-25.0,0.0)
ServoPos2 = (25.0,0.0)
ServoNullAngle1 = 135.0
ServoNullAngle2 = 45.0
leverLen1a = 100.0
leverLen1b = 100.0
leverLen2a = 105.0
leverLen2b = 115.0
...
def calc_servo_angles_for_target(targetpoint):
	...
	j2 = circles_cut(ServoPos2,leverLen2a,targetpoint,leverLen2b)
	a1=tuple(clampangle_deg(math.atan2(p[1],p[0])*180.0/math.pi-ServoNullAngle1) for p in j1)
	a2=tuple(clampangle_deg(math.atan2(p[1],p[0])*180.0/math.pi-ServoNullAngle2) for p in j2)
	if len(a1)>0:
		a1 = max(a1) # first joint out to the left
		if a1<-SERVO_LIMIT_DEG or a1>SERVO_LIMIT_DEG: a1=None
	else:
		a1 = None
	if len(a2)>0:
		a2 = min(a2) # second joint out to the right
